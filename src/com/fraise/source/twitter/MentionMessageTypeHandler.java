...
public class MentionMessageTypeHandler extends AbstractTwitterMessageTypeHandler {

    private TwitterWrapper twitterWrapper;

    public MentionMessageTypeHandler(TwitterWrapper twitterWrapper) {
    ...
    protected Collection<? extends PoetryTweet> convertToPoetryTweets(ResponseList<? extends TwitterResponse> messages,
        ...
        if (messages != null) {
            for (TwitterResponse tr : messages) {
                ...
                PoetryTweet pTweet = new PoetryTweet(message.getId(), message.getText(), message.getUser().getName(),
    ...
    protected ResponseList<? extends TwitterResponse> getMessagesOfSomeType(int page, int pageSize)
        ...
        return twitterWrapper.getMentions(new Paging(page, pageSize));
    }

    @Override
    protected ResponseList<? extends TwitterResponse> getMessagesOfSomeTypeSince(int page, int pageSize, long sinceId)
            throws TwitterException {
        return twitterWrapper.getMentions(new Paging(page, pageSize, sinceId));
    }
